function code = heat_barrier_smr
% heat_barrier_smr — heat barrier only version
%
% Runs ONLY two trial types:
...
% Hardware/save structure unchanged from original heat_basic_conditioning_wz
% (virmenInit/Run/End, niIO_3, vr.data arrays).
%
% Key parameters (adjust as needed):
%   travel_integration_time : seconds fly moves freely before barrier is placed
...
% ── Data arrays ───────────────────────────────────────────────────────────
% barJump kept for pipeline compatibility — always 0 (no bar jumps)
vr.data.barJump               = nan(1, vr.trialDuration*61);
vr.data.heat                  = nan(1, vr.trialDuration*61);
vr.data.currTt                = nan(1, vr.trialDuration*61);
vr.data.heat_barrier_distance = nan(1, vr.trialDuration*61);
vr.data.control_trial         = nan(1, vr.trialDuration*61);

% ── Live figure ───────────────────────────────────────────────────────────
vr.liveFig = figure('Position',[1200 200 700 800]);
subplot(3,2,1:4);
vr.h1 = plot(1:80000, nan(1,80000), 'k.');   % no heat
hold on;
vr.h2 = plot(1:80000, nan(1,80000), 'r.');   % heat on
...
% Full heat at barrier line, zero once inside safe zone
...
% Heat output is zeroed in the runtime (control_trial_no_heat flag).
i = 2;
vr.tt(i).ind                        = i;
...
% ── Trial list: interleave heat and control ───────────────────────────────
if vr.accom == 1
    % Accommodation only — closed-loop bar, no heat, no barrier
    vr.trial_type_list = [3 3 3 3 3 3 3 3 3 3];
else
    % Start with accommodation, then alternate heat barrier / control
    n_repeats          = 20;
    n_accom            = 3;
...
% ── Forward speed (monitoring only — not used for heat in barrier trials) ─
...
if newTrial == 1
    ...
    % Control status set directly from trial type
    % Type 1 = heat active, Type 2 = control (no heat)
    vr.control_trial_no_heat = (vr.current_trial_type == 2);
    disp(['Control trial (no heat output): ' num2str(vr.control_trial_no_heat)]);
end
...
if ~vr.heat_barrier_active
    ...
    % Phase 1: integration window
    % Fly moves freely, no heat, barrier not yet placed
    ...
    % Phase 2: barrier active
    % Compute signed distance (positive = fly is behind barrier / in heat zone)
    fly_to_barrier         = vr.heat_barrier_point - vr.position(1:2);
    vr.distance_to_barrier = dot(fly_to_barrier, vr.heat_barrier_normal);
...
% Record what heat would have been, then zero for control trials
...
if mod(vr.iterations,20) == 0
    ...
    % Trajectory dot: red = heat on, black = off/control
    ...
    if vr.heat_barrier_active
        ...
        % Grey overlay = control, red = heat active
        if vr.control_trial_no_heat
        ...
        % heat_barrier_normal is already a unit vector; scale for visibility
...
vr.data.barJump               = rmmissing(vr.data.barJump);
vr.data.heat                  = rmmissing(vr.data.heat);
vr.data.currTt                = rmmissing(vr.data.currTt);
vr.data.heat_barrier_distance = rmmissing(vr.data.heat_barrier_distance);
vr.data.control_trial         = rmmissing(vr.data.control_trial);
