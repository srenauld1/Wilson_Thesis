function save_fly_data(daq, ts, varargin)
...
%   fly_identifier - unique ID for this fly
...
%   varargin - more fieldname,data pairs
    ...
    if ischar(varargin{1}) || isstring(varargin{1})
        % dff_motion omitted; arguments are daq, ts, fly_identifier, savepath, ...
        dff_motion = [];
        fly_identifier = varargin{1};
        savepath = varargin{2};
        extra_args = varargin(3:end);
    else
        % dff_motion present; arguments are daq, ts, dff_motion, fly_identifier, savepath, ...
        dff_motion = varargin{1};
        fly_identifier = varargin{2};
        savepath = varargin{3};
