function a2p_data = process_pause_data(a2p_data, savepath, box, split, replay_only)
...
%% re-allocate values
N = numel(a2p_data.dq(1).t);

pauses        = zeros(1, N);
open_loop_f2b = zeros(1, N);
open_loop_b2f = zeros(1, N);
reverse_gain  = zeros(1, N);
dark          = zeros(1, N);
...
disp('Zoom/pan as needed, then click and drag to draw a rectangle for each replay interval.');
disp('When you are finished, just single-click on the plot and press Enter without drawing a rectangle.');
...
while true
    ...
    % Visually mark selected interval
    plot(a2p_data.dq(1).t(indices), visual_pattern_velocity(indices),...
        'ko', 'MarkerFaceColor','y','DisplayName','Hand-picked Replay');
    delete(h);
end
...
%% segment out recording (for replay) (by hand)

% Initialize reverse_gain to 0, or load if it already exists
if ~isfield(a2p_data, 'recording') || isempty(a2p_data.recording)
    recording = zeros(size(a2p_data.dq(1).t));
else
    recording = a2p_data.recording;
end
...
disp('Zoom/pan as needed, then click and drag to draw a rectangle for each Recording interval.');
disp('When you are finished, just single-click on the plot and press Enter without drawing a rectangle.');

while true
    h = drawrectangle('Label','Recording','Color','m');
    wait(h); % Wait for rectangle to be created/finished
    rectpos = h.Position;
    % If rectangle has zero width, break out
    if rectpos(3) == 0
...
% Optionally, update struct if storing
a2p_data.recording = recording;

%% segment out dark periods (by hand) - panels broken
do_dark = input('Do you want to hand-pick dark periods? Enter 1 for YES, 0 for NO: ');

if do_dark == 1
    %% segment out dark periods (by hand) - panels broken

    % Initialize reverse_gain to 0, or load if it already exists
    if ~isfield(a2p_data, 'dark') || isempty(a2p_data.dark)
        dark = zeros(size(a2p_data.dq(1).t));
    else
        dark = a2p_data.dark;
    end

    figure; hold on
    plot(a2p_data.dq(1).t, dark, 'y', 'LineWidth',1.5)
    plot(a2p_data.dq(1).t, visual_pattern_velocity, 'b')
    xlabel('Time (s)');
    legend('Darkness','Visual Pattern Velocity');
    title('Darkness (yellow) and Pattern Velocity (blue)');
    set(gcf, 'Color', 'w');

    disp('Zoom/pan as needed, then click and drag to draw a rectangle for each Darkness interval.');
    disp('When you are finished, just single-click on the plot and press Enter without drawing a rectangle.');

    while true
        h = drawrectangle('Label','Darkness','Color','m');
        wait(h); % Wait for rectangle to be created/finished
        rectpos = h.Position;
        % If rectangle has zero width, break out
        if rectpos(3) == 0
        ...
        % Visually mark selected interval
        plot(a2p_data.dq(1).t(indices), visual_pattern_velocity(indices),...
            'ko', 'MarkerFaceColor','y','DisplayName','Hand-picked Darkness');
        delete(h);
    end

    % Mask out dark if any of these other periods:
    bad_indices = (pauses == 1) | (open_loop_f2b == 1) | (replay == 1) | (reverse_gain == 1);
    dark(bad_indices) = 0;
...
if replay(1)==1, replay_starts = [1; replay_starts]; end
if replay(end)==1, replay_ends = [replay_ends; length(replay)]; end


% Get recording start/stop intervals
recording_on = [false; diff(recording(:))]==1;
recording_off = [false; diff(recording(:))]==-1;
recording_starts = find(recording_on);
recording_ends = find(recording_off);

if recording(1)==1, recording_starts = [1; recording_starts]; end
if recording(end)==1, recording_ends = [recording_ends; length(recording)]; end

% if panels broke
% Get dark start/stop intervals
if do_dark
    dark_on = [false; diff(dark(:))]==1;
    dark_off = [false; diff(dark(:))]==-1;
    dark_starts = find(dark_on);
    dark_ends = find(dark_off);
    
    if dark(1)==1, dark_starts = [1; dark_starts]; end
    if dark(end)==1, dark_ends = [dark_ends; length(dark)]; end
end
...
for k = 1:numel(signals)
    ...
    for i = 1:length(replay_starts)
        ...
        for i = 1:length(recording_starts)
        patch([time(recording_starts(i)), time(recording_ends(i)), time(recording_ends(i)), time(recording_starts(i))], ...
            [yl(1), yl(1), yl(2), yl(2)], [0.8 0.8 0.8], 'EdgeColor','none', 'FaceAlpha',0.5);
    end
    if do_dark
        for i = 1:length(dark_starts)
            patch([time(dark_starts(i)), time(dark_ends(i)), time(dark_ends(i)), time(dark_starts(i))], ...
                [yl(1), yl(1), yl(2), yl(2)], [1 1 0.8], 'EdgeColor','none', 'FaceAlpha',0.5);
        end
    end
    % Plot the trace
...
scatter(ball_forward_velocity(recording==1 & select), dff(recording==1 & select), 20, 'black', 'filled', 'DisplayName', 'Recording');
scatter(ball_forward_velocity(replay==1 & select), dff(replay==1 & select), 20, 'yellow', 'filled', 'DisplayName', 'Reverse gain');
if do_dark
    scatter(ball_forward_velocity(dark==1 & select), dff(dark==1 & select), 20, 'black', 'filled', 'DisplayName', 'Dark');
end
...
if do_dark
    idx_closed_loop = (idx_closed_loop ==1) & (dark == 0);
    idx_dark = (dark == 1);
end
figure; hold on
...
%% scatter plot for visual pattern and dff
...
scatter(visual_pattern_velocity(idx_replay), dff(idx_replay), 20, 'yellow', 'filled', 'DisplayName', 'Replay');
scatter(visual_pattern_velocity(idx_recording), dff(idx_recording), 20, 'black', 'filled', 'DisplayName', 'Recording');

if do_dark
    scatter(visual_pattern_velocity(idx_dark), dff(idx_dark), 20, 'black', 'filled', 'DisplayName', 'Reverse gain');
end
...
save_plot_with_title_as_filename('scatterplot', 'record_replay', savepath);

%% now look at time across closed loop periods
% Logical mask for included points
select = ball_forward_velocity > 1;
mask = pauses==0 & select & open_loop_f2b==0 & reverse_gain==0;
t = a2p_data.dq(1).t;

% Extract values
x = ball_forward_velocity(mask);
y = dff(mask);
tvals = t(mask); % Use your time array here

% Normalize t to [0,1] for color mapping
tvals_norm = (tvals - min(tvals)) / (max(tvals) - min(tvals) + eps); % avoid zero division

% Create a blue colormap: light blue (e.g. [0.7 0.85 1]) to dark blue ([0 0 0.8])
npoints = numel(tvals_norm);
map = [linspace(1,0,npoints)' linspace(0,0,npoints)' linspace(0,1,npoints)'];

% For each point, pick the color corresponding to its normalized t
% (or you can just sort by t so colors go from light to dark in x-axis order)
[~, sort_idx] = sort(tvals_norm); % optional, for continuous transition

figure;
hold on
scatter(x(sort_idx), y(sort_idx), 20, map, 'filled'); % scatter with RGB colors for each point
colormap(map);
colorbar;
caxis([min(tvals) max(tvals)]);
xlabel('Forward velocity');
ylabel('dF/F');
title('dF/F vs Forward Velocity (blue darkens as t increases)');
set(gcf, 'Color', 'w');
