...
%% Plot the dff and pattern
    ...
    for i = 3:length(change_points) - 3
        ...
        if clockwise_motion(segment_indices(1)) > 0
            ...
            derivative_of_position = diff(visual_heading);
            dff_cw_motion{end+1} = dff(extended_segment_indices);
            pattern_cw_motion{end+1} = visual_heading(extended_segment_indices);

            % changed to totalspeed_supp
            upsample_factor = length(time_supp)/length(time);
            extended_segment_indices_supp = round(pre_motion_point*upsample_factor):round(post_motion_point*upsample_factor);
            total_speed_cw_motion{end+1} = totalspeed_supp(extended_segment_indices_supp);
            forward_speed_cw_motion{end+1} = ball_forward_velocity_supp(extended_segment_indices_supp);
            rotational_speed_cw_motion{end+1} = abs(ball_yaw_velocity_supp(extended_segment_indices_supp));
            times_cw_motion{end+1} = time(extended_segment_indices);
    ...
    for i = 1:length(dff_cw_motion)
        plot(time(1:length(dff_cw_motion{i})), dff_cw_motion{i}, 'Color', [0.5 1 0.5], 'LineWidth', 1);  % Lighter green
        xline(time(motion_on_markers(1)), 'k--', 'LineWidth', 1.5);  % Dashed black line at motion-on marker
        xline(time(motion_on_markers(1))+stimlen, 'k--', 'LineWidth', 1.5);
    end
    
    % Plot the average velocity for "motionon" in bold green
    plot(time(1:length(dff_cw_motion{i})), average_dff, 'g', 'LineWidth', 3, 'DisplayName', 'Average DFF');
    ...
    for i = 1:length(dff_cw_motion_norm)
        plot(time(1:length(dff_cw_motion_norm{i})), dff_cw_motion_norm{i}, 'Color', [0.5 0.5 0.5], 'LineWidth', 1);  % Lighter blue
        xline(time(motion_on_markers(1)), 'k--', 'LineWidth', 1.5);  % Dashed black line at motion-on marker
        xline(len_sec + stimlen, 'k--', 'LineWidth', 1.5);
    end

    % Plot the average velocity for "motionon" in bold green
    plot(time(1:length(dff_cw_motion{i})), average_cw_dff_norm, 'black', 'LineWidth', 3, 'DisplayName', 'Average DFF');
    ...
    for i = 1:length(pattern_cw_motion)
        plot(time(1:length(pattern_cw_motion{i})), pattern_cw_motion{i}, 'Color', [0.5 0.5 1], 'LineWidth', 1);  % Lighter blue
        xline(time(motion_on_markers(1)), 'k--', 'LineWidth', 1.5);  % Dashed black line at motion-on marker
        xline(len_sec + stimlen, 'k--', 'LineWidth', 1.5);
    end
    ...
    for i = 3:length(change_points) - 3
        ...
        if counterclockwise_motion(segment_indices(1)) > 0
            ...
            pattern_ccw_motion{end+1} = visual_heading(extended_segment_indices);
            
            % changed to totalspeed_supp
            upsample_factor = length(time_supp)/length(time);
            extended_segment_indices_supp = round(pre_motion_point*upsample_factor):round(post_motion_point*upsample_factor);
            total_speed_ccw_motion{end+1} = totalspeed_supp(extended_segment_indices_supp);
            forward_speed_ccw_motion{end+1} = ball_forward_velocity_supp(extended_segment_indices);
            rotational_speed_ccw_motion{end+1} = abs(ball_yaw_velocity_supp(extended_segment_indices_supp));
            times_ccw_motion{end+1} = time(extended_segment_indices);
    ...
    for i = 1:length(dff_ccw_motion)
        plot(time(1:length(dff_ccw_motion{i})), dff_ccw_motion{i}, 'Color', [0.5 1 0.5], 'LineWidth', 1);  % Lighter green
        xline(time(ccw_motion_on_markers(1)), 'k--', 'LineWidth', 1.5);  % Dashed black line at motion-on marker
        xline(time((ccw_motion_on_markers(1)))+stimlen, 'k--', 'LineWidth', 1.5);
    end
    
    % Plot the average velocity for "motionon" in bold green
    plot(time(1:length(dff_ccw_motion{i})), CCW_average_dff, 'g', 'LineWidth', 3, 'DisplayName', 'Average DFF');
    ...
    for i = 1:length(dff_ccw_motion_norm)
        plot(time(1:length(dff_ccw_motion_norm{i})), dff_ccw_motion_norm{i}, 'Color', [0.5 0.5 0.5], 'LineWidth', 1);  % Lighter blue
        xline(time(motion_on_markers(1)), 'k--', 'LineWidth', 1.5);  % Dashed black line at motion-on marker
        xline(len_sec + stimlen, 'k--', 'LineWidth', 1.5);
    end

    % Plot the average velocity for "motionon" in bold green
    plot(time(1:length(dff_ccw_motion{i})), average_ccw_dff_norm, 'black', 'LineWidth', 3, 'DisplayName', 'Average DFF');
    ...
    for i = 1:length(pattern_ccw_motion)
        plot(time(1:length(pattern_ccw_motion{i})), pattern_ccw_motion{i}, 'Color', [0.5 0.5 1], 'LineWidth', 1);  % Lighter blue
        xline(time(ccw_motion_on_markers(1)), 'k--', 'LineWidth', 1.5);  % Dashed black line at motion-on marker
        xline(time((ccw_motion_on_markers(1)))+stimlen, 'k--', 'LineWidth', 1.5);
    end
    ...
    % Plot cw rotation
    for i = 1:length(dff_cw_motion)
        plot(time(1:length(dff_cw_motion{i})), dff_cw_motion{i}, 'Color', [0.5 0.5 1], 'LineWidth', 1);  % Lighter blue
        xline(time(motion_on_markers(1)), 'k--', 'LineWidth', 1.5);  % Dashed black line at motion-on marker
        xline(time((motion_on_markers(1)))+stimlen, 'k--', 'LineWidth', 1.5);
    end

    % Plot the average velocity for cw in blue
    plot(time(1:length(dff_cw_motion{i})), average_dff, 'b', 'LineWidth', 3, 'DisplayName', 'Average DFF');
    ...
    for i = 1:length(total_speed_ccw_motion)
        plot(time_supp(1:length(total_speed_ccw_motion{i})), total_speed_ccw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
    ...
    for i = 1:length(total_speed_cw_motion)
        plot(time_supp(1:length(total_speed_cw_motion{i})), total_speed_cw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
        %plot(daq.t(1:length(total_speed_cw_motion{i})), total_speed_cw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
    ...
    plot(time_supp(1:size(ave_speed, 2)), ave_spd, 'm', 'LineWidth', 3, 'DisplayName', 'Average Speed');
    ...
    for i = 1:length(forward_speed_ccw_motion)
        plot(time(1:length(forward_speed_ccw_motion{i})), forward_speed_ccw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
    ...
    for i = 1:length(forward_speed_cw_motion)
        plot(time(1:length(forward_speed_cw_motion{i})), forward_speed_cw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
        %plot(daq.t(1:length(forward_speed_cw_motion{i})), forward_speed_cw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
    ...
    for i = 1:length(rotational_speed_ccw_motion)
        plot(time_supp(1:length(rotational_speed_ccw_motion{i})), rotational_speed_ccw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
    ...
    for i = 1:length(rotational_speed_cw_motion)
        plot(time_supp(1:length(rotational_speed_cw_motion{i})), rotational_speed_cw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
        %plot(daq.t(1:length(rotational_speed_cw_motion{i})), rotational_speed_cw_motion{i}, 'Color', [0.8 0 1], 'LineWidth', 1);  % Lighter green
    ...
    for i = 1:length(dff_ccw_motion)
        h = plot(time(1:length(dff_ccw_motion{i})), dff_ccw_motion{i}, 'Color', [0.5 1 0.5], 'LineWidth', 1);
    ...
    for i = 1:length(dff_cw_motion)
        h = plot(time(1:length(dff_cw_motion{i})), dff_cw_motion{i}, 'Color', [0.5 0.5 1], 'LineWidth', 1);
        if i == 1  % Only keep first handle for legend
            h2 = h;
        end
    end
    ...
    h3 = plot(time(1:length_cut), bidirectional_dff, 'm', 'LineWidth', 3);
    
    % Create legend with specific handles and labels
    legend([h1, h2, h3], {label_ccw, label_cw, 'Average'}, 'Location', 'best');
    
    xline(time(ccw_motion_on_markers(1)), 'k--', 'LineWidth', 1.5);
    xline(time((ccw_motion_on_markers(1)))+stimlen, 'k--', 'LineWidth', 1.5);
    ...
    for i = 1:length(moving)
        plot(time(1:length(moving{i})), moving{i}, 'Color', [0.5, 0.5, 0.5], 'LineWidth', 1);
    end
    xline(time(ccw_motion_on_markers(1)), 'k--', 'LineWidth', 1.5);
    xline(time((ccw_motion_on_markers(1)))+stimlen, 'k--', 'LineWidth', 1.5);
    plot(time(1:length(moving_ave)), moving_ave, 'black', 'LineWidth', 3, 'DisplayName', 'Average Opto-On Velocity');
    xlabel('Time (s)');
    ...
    for i = 1:length(still)
        plot(time(1:length(still{i})), still{i}, 'Color', [0.5, 0.5, 0.5], 'LineWidth', 1);
    end
    plot(time(1:length(still_ave)), still_ave, 'black', 'LineWidth', 3, 'DisplayName', 'Average Opto-On Velocity');
    xline(time(ccw_motion_on_markers(1)), 'k--', 'LineWidth', 1.5);
    xline(time((ccw_motion_on_markers(1)))+stimlen, 'k--', 'LineWidth', 1.5);
    xlabel('Time (s)');
    ...
    opto_on = time(ccw_motion_on_markers(1));
    opto_off = time((ccw_motion_on_markers(1)))+stimlen;
...
%% now plotting the dff and kinematics with shaded visual motion
    ...
    hold on
    % Calculate your scaling
    total_padding = max(ball_forward_velocity_supp)-min(ball_forward_velocity_supp);
    clockwise_motion_fordff = (clockwise_motion*total_padding)+min(ball_forward_velocity_supp);
    counterclockwise_motion_fordff = (counterclockwise_motion*total_padding)+min(ball_forward_velocity_supp);
    ...
    for i = 1:length(time)
        if cw_mask(i)
            patch([time(i)-0.5*mean(diff(time)), time(i)+0.5*mean(diff(time)), time(i)+0.5*mean(diff(time)), time(i)-0.5*mean(diff(time))], ...
                  [min(ball_forward_velocity_supp), min(ball_forward_velocity_supp), clockwise_motion_fordff(i), clockwise_motion_fordff(i)], [0.5, 0, 0.5], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
        end
        if ccw_mask(i)
            patch([time(i)-0.5*mean(diff(time)), time(i)+0.5*mean(diff(time)), time(i)+0.5*mean(diff(time)), time(i)-0.5*mean(diff(time))], ...
                  [min(ball_forward_velocity_supp), min(ball_forward_velocity_supp), counterclockwise_motion_fordff(i), counterclockwise_motion_fordff(i)], [1, 0.5, 0], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
        end
    end
    plot(time_supp, ball_forward_velocity_supp, 'k', 'LineWidth', 1.5);  % Plot forward velocity in green
    xlabel('Time (s)');
    ylabel('Forward Velocity (mm/s)');
    title('Forward Velocity');
    grid on;
    upperbound = min(20, max(ball_forward_velocity_supp));
    lowerbound = max(-10, min(ball_forward_velocity_supp));
    ...
    hold on
    % Calculate your scaling
    total_padding = max(ball_yaw_velocity_supp)-min(ball_yaw_velocity_supp);
    clockwise_motion_fordff = (clockwise_motion*total_padding)+min(ball_yaw_velocity_supp);
    counterclockwise_motion_fordff = (counterclockwise_motion*total_padding)+min(ball_yaw_velocity_supp);
    ...
    for i = 1:length(time)
        if cw_mask(i)
            patch([time(i)-0.5*mean(diff(time)), time(i)+0.5*mean(diff(time)), time(i)+0.5*mean(diff(time)), time(i)-0.5*mean(diff(time))], ...
                  [min(ball_yaw_velocity_supp), min(ball_yaw_velocity_supp), clockwise_motion_fordff(i), clockwise_motion_fordff(i)], [0.5, 0, 0.5], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
        end
        if ccw_mask(i)
            patch([time(i)-0.5*mean(diff(time)), time(i)+0.5*mean(diff(time)), time(i)+0.5*mean(diff(time)), time(i)-0.5*mean(diff(time))], ...
                  [min(ball_yaw_velocity_supp), min(ball_yaw_velocity_supp), counterclockwise_motion_fordff(i), counterclockwise_motion_fordff(i)], [1, 0.5, 0], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
        end
    end
    plot(time_supp, ball_yaw_velocity_supp, 'r', 'LineWidth', 2);  % Plot rotational velocity in blue
    xlabel('Time (s)');
    ylabel('Rotational Velocity (rad/s)');
    title('Rotational Velocity');
    grid on;
    upperbound = min(300, max(ball_yaw_velocity_supp));
    lowerbound = max(-300, min(ball_yaw_velocity_supp));
    ylim([lowerbound, upperbound]);
...
%% now plot ccw and cw motion on same axis scale
    ...
    for i = 1:length(dff_ccw_motion)
        plot(time(1:length(dff_ccw_motion{i})), dff_ccw_motion{i}, 'Color', [1 0.7 0.3], 'LineWidth', 1);
        xline(time(ccw_motion_on_markers(1)), 'k--', 'LineWidth', 1.5);
        xline(time((ccw_motion_on_markers(1)))+stimlen, 'k--', 'LineWidth', 1.5);
    end
    
    plot(time(1:length(dff_ccw_motion{i})), CCW_average_dff, 'Color', [1 0.5 0], 'LineWidth', 3, 'DisplayName', 'Average DFF');
    ...
    hold on;
    for i = 1:length(dff_cw_motion)
        plot(time(1:length(dff_cw_motion{i})), dff_cw_motion{i}, 'Color', [0.7 0.3 1], 'LineWidth', 1);
        xline(time(motion_on_markers(1)), 'k--', 'LineWidth', 1.5);
        xline(time((motion_on_markers(1)))+stimlen, 'k--', 'LineWidth', 1.5);
    end
    
    plot(time(1:length(dff_cw_motion{i})), average_dff, 'Color', [0.5 0 1], 'LineWidth', 3, 'DisplayName', 'Average DFF');
