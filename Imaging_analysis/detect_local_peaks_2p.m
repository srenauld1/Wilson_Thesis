function daq = detect_local_peaks_2p(daq, varargin)
    % Parse inputs
    p = inputParser;
    addRequired(p, 'ts');
    ...
    filtered_signal = daq.absolute_circular_diff(valid_times);
    ...
    daq.jump_detected = zeros(size(time_values));
    daq.jump_detected(peak_indices) = 1;
end